...
from telegram_data_base import db_config
...
def verify():
    ...
    if request.args.get('Status') == 'OK':
        ...
        if result.json()["data"]["code"] == 100:
            ...
            with telegram_data_base.pymysql.connect(**db_config) as connection:

                with connection.cursor() as cursor:
                    cursor.execute(f"UPDATE users SET wallet = wallet + {amount} WHERE id = {user[request.args['Authority']]}")
                    connection.commit()
